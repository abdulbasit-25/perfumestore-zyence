# Master Prompt: Full Design & UX Overhaul — Zyence E-Commerce Platform
...
- **Brand:** "Zyence" — a fragrance house creating composed eau de parfum, attars, and home scents. Positioning is precise, modern, editorial, and sensory. Key selling points: pay-on-delivery, considered composition, small-batch production, and reliable delivery.
- **Existing surfaces:** Home, Shop (with category filters), Product detail, Cart, Checkout, Account/Orders, Login, About ("Studio"), Contact, Admin/Manager dashboard (role-gated), legal pages, newsletter signup, FAQ accordion, Instagram feed strip, a "Store Assistant" chat widget.
...
Your job: **audit, then rebuild the design system and UI layer** across the entire app — customer-facing and admin — without breaking existing routes, data flow, or business logic. This is a _design and front-end quality_ pass, not a rewrite of business logic.
...
4. **One design system, two "modes."** Customer-facing pages keep the warm/editorial/artisan aesthetic. The admin dashboard uses the _same design tokens_ (colors, type scale, radii, spacing, motion curves) but a denser, utilitarian layout language (tables, data cards, forms) — same DNA, different density. They should feel like siblings, not two unrelated products.
...
- **Container/grid system:** a consistent max-width + gutter system (I saw `max-w-[1500px]` in the header — confirm this is the _one_ container width used everywhere, or standardize it).
...
Build these as a proper internal library (e.g. `/components/ui`) if they don't already exist in a consistent form. Use shadcn/ui-style primitives as a base where sensible, then re-skin to the Zyence aesthetic:
...
- **Header/Nav:** Sticky header, correct active-link highlighting per section _and_ category (not just per route — e.g. "Apparel" and "Ceramics" both route to `/shop` with different query params, so active state must check the query, not just the path). Mobile drawer with backdrop, scroll-lock, escape-to-close, animated icon swap. Add a subtle scroll-shadow/border-strengthen effect once the user scrolls down. Cart badge with live count and micro-animation on item add.
...
- **About/Studio, Contact, Legal pages:** Long-form content should use a consistent editorial reading-width container, good typographic rhythm; contact form uses shared form components + success/error toast.
