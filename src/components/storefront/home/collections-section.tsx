      ...
      {categories.length > 0 ? (
        <div className="border-t border-hairline">
          {categories.map((category, index) => (
            <CollectionRow key={category.id} category={category} index={index} />
          ))}
        </div>
      ) : (
        <div className="border-t border-b border-hairline py-16 text-center text-sm text-muted-foreground">
          Collections are being restocked — check back shortly.
        </div>
      )}
    </section>
  );
...
/** One category row: index, name, description, and the arrow affordance. */
function CollectionRow({ category, index }: { category: Category; index: number }) {
  return (
    <Link
      to="/shop"
