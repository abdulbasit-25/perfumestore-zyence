...
function navSearch(item: NavItem) {
  return "search" in item ? (item.search as never) : ({} as never);
}
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-hairline bg-background/80 backdrop-blur-md shadow-[0_10px_30px_-24px_rgba(0,0,0,0.14)]">
      <div className="section-shell flex h-16 items-center gap-4 md:gap-6">
        ...
        <HeaderActions user={user} cartCount={count} />
      </div>
...
/** Theme toggle, admin shortcut, account, and cart — the right-hand action cluster. */
